  ...
  try {
    ...
    function extractVouchers() {
      ...
      [...allEls, ...byText].forEach(el => {
        ...
        if (rmMatch) {
          vouchers.push({ type: 'fixed', amount: parseFloat(rmMatch[1]), raw: txt });
          return;
        }
        // Percentage: 5% off
        const pctMatch = txt.match(/(\d+(?:\.\d+)?)\s*%\s*(?:off|OFF)/i);
        if (pctMatch) {
          vouchers.push({ type: 'percent', amount: parseFloat(pctMatch[1]), raw: txt });
        }
      });
      // Deduplicate
      const seen = new Set();
      return vouchers.filter(v => {
        const k = `${v.type}:${v.amount}`;
    ...
    // Best fixed voucher (largest RM off)
    ...
    // Effective prices after best voucher
    result.variants = result.variants.map(v => ({
      ...v,
      effectivePrice: v.price != null ? Math.max(0, v.price - result.bestVoucherAmount) : null,
    }));
